 ...
 * order); passed by address to I2C_StructInit()/I2C_Init().  The three DMA
 * request-level fields sit between I2CFilter and I2CAckAddr1 only on chips
 * that carry them (AMEBA_I2C_HAS_DMA_FIELDS, see ameba_i2c_chip.h); omitting
 * them elsewhere keeps this struct byte-for-byte identical to the fwlib one.
 */
